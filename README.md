...
* Minimal admin settings page with license status, diagnostics, and AI Sentinel audit logs
* Fully Internationalized (i18n) — Natively supports 150+ languages using WordPress translation standards
* Self-contained: no third-party identity provider or phone number; Premium accounts do not require a user-supplied email address
...
2. Choose the `sovereign-ai-overseer-premium.zip` file and click **Install Now**.
3. Click **Activate**.
...
Sovereign AI Overseer doesn't just passively log attacks; it queues suspicious authentication activity for Gemini analysis, then applies a deterministic Layer 7 mitigation policy to the resulting risk score.
...
* Added: REST API Webhook Endpoint (`/wp-json/sovereign-ai/v1/agent-webhook`) allowing zero-touch background processing of support tickets and marketing tasks via Zapier/Make.com.
